...
function App() {
  ...
  const [drawingMode, setDrawingMode] = useState<DrawingMode>(DRAWING_MODES.DRAW);
  ...
  const { getTouchPos, getPointerPos, drawLine, lastTouchRef } = useTouchDrawing();
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (canvas && lastTouchRef.current) {
      ...
      drawLine(ctx!, lastTouchRef.current, currentPos, color, lineWidth, drawingMode);
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    if (!isDrawing || (e.pointerType !== 'pen' && e.pointerType !== 'touch')) return;
  ...
  const generateImage = async () => {
    ...
    if (canvas) {
      try {
        ...
        // const response = await fetch('your-backend-url/calculate', {
        //   method: 'POST',
        //   headers: {
        ...
        setTimeout(() => {
          console.log('Image data sent to backend:', imageData.substring(0, 100) + '...');
  ...
  return (
    <div className="relative w-full h-screen bg-gray-900 overflow-hidden">
      ...
      {/* Instructions */}
      <div className={`absolute ${isMobile ? 'bottom-20' : 'bottom-4'} left-4 right-4 text-center text-white/60 text-sm`}>
        {isMobile 
